...
1. Removes records with 'None' in the result column
...
none_df = df[df['result'] == 'None'] 
removed_df = pd.concat([removed_df, none_df]) 
df = df[df['result'] != 'None']

# Process duplicate records
dup_notes = df[df.duplicated('noteid', keep=False)]['noteid'].unique() 
for note in dup_notes:
    note_df = df[df['noteid'] == note]

    if 'Prior' in note_df['result'].values:
        keep_index = note_df[note_df['result'] == 'Prior'].index[0]
    elif 'Disc' in note_df['result'].values:
        keep_index = note_df[note_df['result'] == 'Disc'].index[0]
